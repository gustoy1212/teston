-- [[ 👑 OMNI-LEGACY V9: SNIPER & INFINITE ]] --
-- Correção do Filtro de Mobs (Rigoroso) + Alcance Infinito
...
getgenv().OmniConfig = {
    ...
    TargetMode = "SELECT",  -- [FIX] Começa já no modo SELEÇÃO para segurança
    SelectedMobs = {},      -- Lista de permitidos
    
    HitboxSize = 70,        -- Hitbox
    MagnetDist = 5,         -- Distância do player
...
MainFrame.Position = UDim2.new(0.5, -225, 0.12, 0)
MainFrame.BackgroundColor3 = Color3.fromRGB(10, 10, 12)
MainFrame.BorderColor3 = Color3.fromRGB(0, 255, 255) -- Ciano V9
...
Title.Text = "👑 OMNI-LEGACY V9 (SNIPER)"
...
TabContainer.Position = UDim2.new(0, 0, 0.086, 0)
...
-- 2. MODO ALVO (CORRIGIDO)
local ModeBtn = CreateToggle("🎯 FILTRO", Color3.fromRGB(255, 100, 0), function()
    if getgenv().OmniConfig.TargetMode == "ALL" then
        getgenv().OmniConfig.TargetMode = "SELECT"
        return true, "🎯 FILTRO: [SÓ DA LISTA]"
    else
        getgenv().OmniConfig.TargetMode = "ALL"
        return false 
    end
end)
-- Define texto inicial correto
ModeBtn.Text = "🎯 FILTRO: [SÓ DA LISTA]" -- Começa ativo no Select
ModeBtn.BackgroundColor3 = Color3.fromRGB(255, 100, 0)
ModeBtn.TextColor3 = Color3.fromRGB(255, 255, 255)
getgenv().OmniConfig.TargetMode = "SELECT" -- Força início seguro
...
StatusLbl.Text = "Aperte para encontrar mobs..."
...
ScanBtn.MouseButton1Click:Connect(function()
    ...
    StatusLbl.Text = "Lista atualizada: " .. Count .. " nomes."
end)
...
local function IsTargetValid(mob)
    ...
    -- MODO SÓ DA LISTA (RIGOROSO)
    if getgenv().OmniConfig.TargetMode == "SELECT" then
        if getgenv().OmniConfig.SelectedMobs[mob.Name] == true then
            return true
        else
            return false -- REJEIÇÃO IMEDIATA
        end
    end
    return false
end
                ...
                for _, obj in ipairs(Workspace:GetDescendants()) do
                    if obj:IsA("Humanoid") and obj.Parent ~= Char and obj.Health > 0 then
                        ...
                        if IsTargetValid(mob) then
                            ...
                            local r = mob:FindFirstChild("HumanoidRootPart")
                            if r then
                                -- TELEPORTE SEM CHECAR DISTÂNCIA
                                r.CFrame = PullPos
...
RunService.Heartbeat:Connect(function()
    ...
    if getgenv().OmniConfig.MagnetMode == "SINGLE" and Root then
        local closest, dist = nil, 999999 -- Distância infinita
        for _, obj in ipairs(Workspace:GetDescendants()) do
            if obj:IsA("Humanoid") and obj.Parent ~= Char and obj.Health > 0 then
                local mob = obj.Parent
                if IsTargetValid(mob) then
    ...
    if getgenv().OmniConfig.LootVacuum and Root then
        pcall(function()
             for _, v in pairs(Workspace:GetDescendants()) do
                ...
                elseif (v.Name:lower():find("drop") or v.Name:lower():find("coin") or v.Name:lower():find("potion")) and v:IsA("BasePart") then
