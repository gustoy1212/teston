-- [[ 👑 OMNI-LEGACY V4: AFK GOD EDITION ]] --
-- Adicionado: Aspirador de Loot, Auto-Baú e Imunidade a Stun.
...
getgenv().OmniConfig = {
    ...
    DPS_Mode = false,     
    AnimSpeed = 50,
    DeleteStates = true,  -- (DPS + Anti-Stun)
    AutoClick = false,
    
    -- Novos Recursos
    LootVacuum = false,   -- Puxar itens do chão
    AutoChest = false,    -- Abrir baús sozinho
    GodBody = false       -- Remove Stun e Knockback
}
...
MainFrame.Size = UDim2.new(0, 340, 0, 380) -- Maior para caber tudo
...
Title.Text = "👑 OMNI-LEGACY V4 (AFK GOD)"
...
-- // LÓGICA DE DETECÇÃO DE MOBS (DO LEGACY V3) //
...
RunService.Heartbeat:Connect(function()
    ...
    if getgenv().OmniConfig.MagnetMode ~= "OFF" then
        ...
        if Folder then
            if getgenv().OmniConfig.MagnetMode == "SINGLE" then
                -- (Lógica 1v1 omitida para economizar espaço, igual V3)
    ...
    -- === 2. LOOT VACUUM (NOVO) ===
    if getgenv().OmniConfig.LootVacuum then
        pcall(function()
            -- Varre o mapa procurando Drops
            for _, obj in pairs(Workspace:GetDescendants()) do
                -- Verifica se é Tool (Item pegável) ou se tem nome de item
                if (obj:IsA("Tool") or obj.Name:lower():find("drop") or obj.Name:lower():find("coin") or obj.Name:lower():find("potion")) and obj:IsA("BasePart") or obj:IsA("Model") then
                    ...
                    -- Se for uma Tool no chão (Handle)
                    if obj:IsA("Tool") and obj:FindFirstChild("Handle") then
                        obj.Handle.CFrame = MyRoot.CFrame
                    -- Se for um modelo de drop (Baseado no seu log: Workspace.Camera.Drops)
                    elseif obj:IsA("BasePart") and (obj.Name == "Handle" or obj.Name == "Part") then
                         -- Verifica se está solto no mundo (não dentro de outro player)
        ...
        pcall(function()
            for _, v in pairs(Workspace:GetDescendants()) do
                if v:IsA("ProximityPrompt") then
                    ...
                    if parentName:find("chest") or parentName:find("bau") or parentName:find("loot") then
                        -- Dispara o prompt automaticamente
...
RunService.RenderStepped:Connect(function()
    ...
    -- === 4. DPS & GOD BODY (ATUALIZADO) ===
    if getgenv().OmniConfig.DPS_Mode or getgenv().OmniConfig.GodBody then
        pcall(function()
            local s = Char:FindFirstChild("States")
            if s then
                -- DPS: Remove o delay do ataque
                if getgenv().OmniConfig.DPS_Mode and s:FindFirstChild("UsingSkill") then 
                    s.UsingSkill:Destroy() 
                end
                
                -- GOD BODY: Remove Stun e Empurrão
